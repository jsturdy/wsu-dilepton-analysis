...
if __name__ == "__main__":
    ...
    parser.add_option("--mcbias", type="int", dest="mcbias",
                      metavar="mcbias", default=0,
                      help="[OPTIONAL] Number of pseudoexperiment to take results from (default is 0)")
    ...
    etaphiexclusivebins = ["EtaPlusPhiMinus","EtaPlusPhiZero","EtaPlusPhiPlus",
                           "EtaMinusPhiMinus","EtaMinusPhiZero","EtaMinusPhiPlus"
                           ]
    etaphiinclusivebins = {"All":     etaphiexclusivebins,
                           "EtaPlus": etaphiexclusivebins[0:3],
                           "EtaMinus":etaphiexclusivebins[3:8],}
    print etaphiinclusivebins
    ...
    testHist = None
    if options.etaphi not in etaphiexclusivebins:
        pass
    else:
        testHist = myInFile.Get("%s/%s%s%s"%(options.etaphi,histName,"PlusCurve",options.etaphi)).Clone("test")

    if options.mcclosure:
        mcBiasBin = options.mcbias
        mcBiasSuffix = "020PseudoData%03d"%(mcBiasBin)
        
        print "%s/%s%s%s%s"%(options.etaphi,histName,"PlusCurve",options.etaphi,mcBiasSuffix)
        plusClosureHistp100 = p100InFile.Get("%s/%s%s%s%s"%(options.etaphi,histName,"PlusCurve",
                                                            options.etaphi,mcBiasSuffix)).Clone("clonep100")
        plusClosureHistp100.Scale(p100top500ScaleFactor)
        plusClosureHistp500 = p500InFile.Get("%s/%s%s%s%s"%(options.etaphi,histName,"PlusCurve",
                                                            options.etaphi,mcBiasSuffix)).Clone("clonep500")

        plusClosureHist  = plusClosureHistp500.Clone("%s%s%s%s_scaling"%(histName,"PlusCurve",
                                                                         options.etaphi,mcBiasSuffix))
    ...
    if options.mcclosure:
        ...
        print "Using %s/%s%s%s%s as reference histograms"%(options.etaphi,histName,"Plus[Minus]Curve",options.etaphi,mcBiasSuffix)

        plusClosureHistp100 = p100InFile.Get("%s/%s%s%s%s"%(options.etaphi,histName,"PlusCurve",
                                                            options.etaphi,mcBiasSuffix)).Clone("plusClosureHistp100")
        plusClosureHistp100.Scale(p100top500ScaleFactor)

        minusClosureHistp100 = p100InFile.Get("%s/%s%s%s%s"%(options.etaphi,histName,"MinusCurve",
                                                             options.etaphi,mcBiasSuffix)).Clone("minusClosureHistp100")
        minusClosureHistp100.Scale(p100top500ScaleFactor)

        plusClosureHistp500  = p500InFile.Get("%s/%s%s%s%s"%(options.etaphi,histName,"PlusCurve",
                                                             options.etaphi,mcBiasSuffix)).Clone("plusClosureHistp500")
        minusClosureHistp500 = p500InFile.Get("%s/%s%s%s%s"%(options.etaphi,histName,"MinusCurve",
                                                             options.etaphi,mcBiasSuffix)).Clone("minusClosureHistp500")
    ...
    for step in range(0,options.biasbins/options.stepsize):
        ...
        if options.debug:
            print "%s/%s%s%sMinusBias%03dMCClosure%03d"%(options.etaphi,histName,"PlusCurve", options.etaphi,
                                                         options.biasbins-step*options.stepsize,options.mcbias)
        plusHistp100  = p100InFile.Get("%s/%s%s%sMinusBias%03dMCClosure%03d"%(options.etaphi,histName,"PlusCurve", options.etaphi,
                                                              options.biasbins-step*options.stepsize,options.mcbias))
        minusHistp100 = p100InFile.Get("%s/%s%s%sMinusBias%03dMCClosure%03d"%(options.etaphi,histName,"MinusCurve",options.etaphi,
                                                              options.biasbins-step*options.stepsize,options.mcbias))
        plusHistp100.Scale( p100top500ScaleFactor)
        minusHistp100.Scale(p100top500ScaleFactor)

        plusHistp500  = p500InFile.Get("%s/%s%s%sMinusBias%03dMCClosure%03d"%(options.etaphi,histName,"PlusCurve", options.etaphi,
                                                            options.biasbins-step*options.stepsize,options.mcbias))
        minusHistp500 = p500InFile.Get("%s/%s%s%sMinusBias%03dMCClosure%03d"%(options.etaphi,histName,"MinusCurve",options.etaphi,
                                                            options.biasbins-step*options.stepsize,options.mcbias))
    ...
    plusHistp100  = p100InFile.Get("%s/%s%s%sPlusBias000MCClosure%03d"%(options.etaphi,histName,"PlusCurve",
                                                                        options.etaphi,options.mcbias))
    minusHistp100 = p100InFile.Get("%s/%s%s%sPlusBias000MCClosure%03d"%(options.etaphi,histName,"MinusCurve",
                                                                        options.etaphi,options.mcbias))
    plusHistp100.Scale(p100top500ScaleFactor)
    minusHistp100.Scale(p100top500ScaleFactor)

    plusHistp500  = p500InFile.Get("%s/%s%s%sPlusBias000MCClosure%03d"%(options.etaphi,histName,"PlusCurve",
                                                                        options.etaphi,options.mcbias))
    minusHistp500 = p500InFile.Get("%s/%s%s%sPlusBias000MCClosure%03d"%(options.etaphi,histName,"MinusCurve",
                                                                        options.etaphi,options.mcbias))
    ...
    for step in range(0,options.biasbins/options.stepsize):
        ...
        plusHistp100  = p100InFile.Get("%s/%s%s%sPlusBias%03dMCClosure%03d"%(options.etaphi,histName,"PlusCurve", options.etaphi,
                                                                             (1+step)*options.stepsize,options.mcbias))
        minusHistp100 = p100InFile.Get("%s/%s%s%sPlusBias%03dMCClosure%03d"%(options.etaphi,histName,"MinusCurve",options.etaphi,
                                                             (1+step)*options.stepsize,options.mcbias))
        plusHistp100.Scale(p100top500ScaleFactor)
        minusHistp100.Scale(p100top500ScaleFactor)

        plusHistp500  = p500InFile.Get("%s/%s%s%sPlusBias%03dMCClosure%03d"%(options.etaphi,histName,"PlusCurve", options.etaphi,
                                                             (1+step)*options.stepsize,options.mcbias))
        minusHistp500 = p500InFile.Get("%s/%s%s%sPlusBias%03dMCClosure%03d"%(options.etaphi,histName,"MinusCurve",options.etaphi,
                                                             (1+step)*options.stepsize,options.mcbias))
