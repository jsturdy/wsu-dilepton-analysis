...
def makeClosurePlots(samplename,etaphi):
    for leg in muonLegs:
        for pm in pmstring:
            jobname = "oct20_thresh10_closure_study_looseMu%s_%s_%s%s"%(leg,etaphi,samplename,pm)
    
            xVals = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            xErrs = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distAvgVsBias   = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distMeanVsBias  = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distSigmaVsBias = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distRMSVsBias   = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distAvgVsBiasErr   = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distMeanVsBiasErr  = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distSigmaVsBiasErr = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            distRMSVsBiasErr   = np.zeros(len(closurebins.keys()),np.dtype('float64'))
            ...
            for pull in range(5):
                pullAvgVsBias.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullMeanVsBias.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullSigmaVsBias.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullRMSVsBias.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullAvgVsBiasErr.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullMeanVsBiasErr.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullSigmaVsBiasErr.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pullRMSVsBiasErr.append(np.zeros(len(closurebins.keys()),np.dtype('float64')))
                pass
            ...
            outfile = r.TFile("%s_output.root"%(jobname),"update")
            outdir  = outfile.mkdir("%s"%(samplename))

            infile = r.TFile("oct20_thresh10/closure_study/%s/%s.root"%(etaphi,jobname),"READ")
            print infile
            for closurebin in closurebins.keys():
                closuredir = infile.Get(closurebin)
                print closuredir
                N_BINS = 500
                outcan = r.TCanvas("outcan","%s%s"%(jobname,closurebin),1280,1024)
                outcan.Divide(2,2)
                outcan.cd(1)
                ...
                for cbin in range(N_BINS):
                    graphname  = "chi2_looseMu%s_%s_closureBin%03d"%(leg,etaphi,cbin)
                    prefitname = "preFitPoly_looseMu%s_%s_closureBin%03d"%(leg,etaphi,cbin)
                    fitname    = "fitPoly_looseMu%s_%s_closureBin%03d"%(leg,etaphi,cbin)
                    graph  = closuredir.Get(graphname)
                    prefit = closuredir.Get(prefitname)
                    fit    = closuredir.Get(fitname)
                    if graph:
                ...
                outcan.cd(2)
                chi2Dist = closuredir.Get("chi2Dist")
                ...
                outcan.cd(3)
                chi2Min = closuredir.Get("chi2Min")
                ...
                outcan.cd(4)
                chi2Width = closuredir.Get("chi2Width")
                ...
                pullcan = r.TCanvas("pullcan","%s%s"%(jobname,closurebin),1280,1024)
                pullcan.Divide(2,3)
                for pull in range(5):
                    singlepullcan = r.TCanvas("singlepullcan","%s%s"%(jobname,closurebin),1280,1024)
                    idx = pull + 1
                    pullcan.cd(idx)
                    chi2Pull = closuredir.Get("chi2Pull%d"%(idx))
                ...
                #raw_input("press enter to continue")
                outcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s.png"%(jobname,closurebin))
                outcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s.pdf"%(jobname,closurebin))
                outcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s.eps"%(jobname,closurebin))
                outcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s.C"  %(jobname,closurebin))
                outcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s.png"%(jobname,closurebin))
    
                pullcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s_pulls.png"%(jobname,closurebin))
                pullcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s_pulls.pdf"%(jobname,closurebin))
                pullcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s_pulls.eps"%(jobname,closurebin))
                pullcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s_pulls.C"  %(jobname,closurebin))
                pullcan.SaveAs("~/public/html/Cosmics/2016/EndpointClosureStudy/%s%s_pulls.png"%(jobname,closurebin))
                pass
            ...
            lastcan.Divide(6,2)
            lastcan.cd(1)
            distAvgVsBiasGraph = r.TGraphErrors(len(closurebins.keys()),xVals,distAvgVsBias,xErrs,distAvgVsBiasErr)
            ...
            distMeanVsBiasGraph = r.TGraphErrors(len(closurebins.keys()),xVals,distMeanVsBias,xErrs,distMeanVsBiasErr)
            ...
            distRMSVsBiasGraph = r.TGraphErrors(len(closurebins.keys()),xVals,distRMSVsBias,xErrs,distRMSVsBiasErr)
            ...
            distSigmaVsBiasGraph = r.TGraphErrors(len(closurebins.keys()),xVals,distSigmaVsBias,xErrs,distSigmaVsBiasErr)
            ...
            sigmaLine = r.TLine(1.2*xVals[0], 1.0, 1.2*xVals[len(closurebins.keys())-1], 1.0)
            sigmaLine.SetLineColor(r.kBlue)
            sigmaLine.SetLineWidth(2)
    
            meanLine  = r.TLine(1.2*xVals[0], 0.0, 1.2*xVals[len(closurebins.keys())-1], 0.0)
            meanLine.SetLineColor(r.kBlue)
            meanLine.SetLineWidth(2)
    
            for pull in range(5):
                pullAvgVsBiasGraph.append(r.TGraphErrors(len(closurebins.keys()),xVals,pullAvgVsBias[pull],xErrs,pullAvgVsBiasErr[pull]))
                ...
                pullMeanVsBiasGraph.append(r.TGraphErrors(len(closurebins.keys()),xVals,pullMeanVsBias[pull],xErrs,pullMeanVsBiasErr[pull]))
                ...
                pullRMSVsBiasGraph.append(r.TGraphErrors(len(closurebins.keys()),xVals,pullRMSVsBias[pull],xErrs,pullRMSVsBiasErr[pull]))
                ...
                pullSigmaVsBiasGraph.append(r.TGraphErrors(len(closurebins.keys()),xVals,pullSigmaVsBias[pull],xErrs,pullSigmaVsBiasErr[pull]))
...
for etaphi in etaphibins:
    samplename = "b40_s5_pt125"
    makeClosurePlots(samplename,etaphi)
    if etaphi == "All":
        steps = [1, 5, 25]
        for step in steps:
            samplename = "b40_s%d_pt125"%(step)
            makeClosurePlots(samplename,etaphi)
            pass
        bins  = [10, 20, 40]
        for bin in bins:
            samplename = "b%d_s5_pt125"%(bin)
            makeClosurePlots(samplename,etaphi)
            pass
        pass
pr.disable()
